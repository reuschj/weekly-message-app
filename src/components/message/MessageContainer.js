...
// This returns an object with various permissions, set based on the current state/props.
// These are passed down to show/hide buttons based on user type / status, etc.
const setPermissions = (state) => {
    ...
    // This sets viewability of the message depending on viewer type and state.
    // Viewers can't see it unless it's published.
    if (userType === contributor || userType === reviewer) {
        newPermissions.allowView = true;
    } else if (userType === viewer && status === published) {
        newPermissions.allowView = true;
    }
    // This ensures that only contributors and reviewers can see the edit status bar.
    // It will be hidden from viewers.
    if (userType === contributor || userType === reviewer) {
        newPermissions.allowViewStatus = true;
    }
    // This sets permissions to edit based on user type and status.
    // Contributors can only edit in draft status.
    // Reviewers can only edit in review or published status. Viewers cannot edit.
